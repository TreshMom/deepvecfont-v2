...
def process(process_id, font_num_p_process, ttf_names, charset, fonts_file_path, sfd_path, opts):
...
def write_glyph_imgs_mp(opts):
    ...
    ttf_names = []  # Инициализация переменной
    ...
    processes = [mp.Process(target=process, args=(pid, font_num_per_process, ttf_names, charset, fonts_file_path, sfd_path, opts)) for pid in range(process_nums)]
